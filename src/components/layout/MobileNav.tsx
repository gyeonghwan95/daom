...
export function MobileNav({ open, onClose }: MobileNavProps) {
  ...
  return (
    ...
    >
      ...
      <nav
        className="absolute right-0 top-0 z-10 flex h-full w-[min(100vw-2.5rem,21rem)] max-w-full flex-col overflow-hidden bg-cream shadow-2xl"
        style={{
          paddingBottom: "calc(7rem + env(safe-area-inset-bottom, 0px))",
        }}
        aria-label="모바일 메뉴"
      >
        <div className="flex shrink-0 items-center gap-1 border-b border-beige-dark px-2 py-2.5">
          {inSubmenu ? (
            <>
              <button
                type="button"
            ...
            <p
              id={titleId}
              className="min-w-0 flex-1 truncate px-2 text-sm font-semibold text-navy"
            >
              {siteConfig.name}
            </p>
          )}
            ...
            >
              <ul className="space-y-0.5 px-2 pt-3">
                {mainNavigation.map((item) => {
                  ...
                  if (hasChildren) {
                    const childCount = item.groups!.reduce(
                    ...
                    return (
                      <li key={item.href}>
                        <button
                          ...
                          aria-haspopup="true"
                          className={[
                            "group flex w-full min-h-12 items-center gap-3 rounded-xl px-3.5 text-left transition-colors",
                            active
                              ? "bg-white font-semibold text-navy shadow-sm ring-1 ring-beige-dark"
                              : "font-medium text-navy/85 active:bg-beige/70",
                          ].join(" ")}
                        >
                          ...
                          <span
                            className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-beige text-navy/55 group-active:bg-beige-dark"
                            aria-hidden
                          >
                            <ChevronRightIcon />
                          </span>
                  ...
                  return (
                    <li key={item.href}>
                      <Link
                        ...
                        className={[
                          "flex min-h-12 items-center rounded-xl px-3.5 text-base no-underline transition-colors",
                          active
                            ? "bg-white font-semibold text-navy shadow-sm ring-1 ring-beige-dark"
                            : "font-medium text-navy/85 active:bg-beige/70",
                        ].join(" ")}
  ...
  return (
    <div className="px-3 pt-3">
      <Link
        href={item.href}
        onClick={onClose}
        className="mb-4 flex items-center gap-3 rounded-2xl border border-beige-dark bg-white px-4 py-3.5 no-underline shadow-[0_1px_2px_rgba(30,58,95,0.04)] active:bg-beige/40"
      >
        ...
        {item.groups!.map((group, groupIndex) => (
          ...
          >
            <h2
              ...
              className="mb-2 px-1 text-[0.6875rem] font-semibold tracking-[0.06em] text-navy/45"
            >
              {group.title}
            </h2>
