...
export function splitIntroParagraphs(
  ...
  const fallback =
    extras[0] ??
    "부산 해운대·센텀에 위치한 다옴법무사사무소 안윤정 법무사가 절차·서류·비용을 항목별로 설명드립니다. 막막할수록 지금 무엇부터 해야 하는지부터 정리하는 것이 우선이라고 생각합니다.";
...
export function ensureMinContent(page: PageData, min = 2000): PageData {
  ...
  const supplement = {
    title: "상담 시 확인하는 내용",
    body: `${page.title} 사건은 서류·관할·기한에 따라 진행 순서가 달라집니다. 다옴법무사사무소는 부산 해운대·센텀에서 상속등기·부동산등기·법인등기·개인회생·파산 등을 다루며, 의뢰인 상황에 맞춰 절차와 비용을 알기 쉽게 설명합니다. 불확실한 부분은 추측으로 단정하지 않고, 확인된 사실을 바탕으로 안내드립니다. 전화·카카오톡·네이버 톡톡으로 간단히 상황을 남겨 주시면 필요한 준비부터 차분히 정리해 드리겠습니다.`,
  };
