...
export function buildPageDataFromLocalLanding(
  ...
  if (page.slug === "부산법인등기") {
    extraSections.unshift({
      title: "부산에서 법인전문 법무사를 찾는 대표자가 먼저 확인할 등기사항",
      body: "최신 등기사항전부증명서와 정관을 대조해 설립·임원변경·본점이전·목적·상호·유상증자·감자·지점·해산·청산·외국인 주주·임원 중 무엇이 필요한지부터 가릅니다. ‘법인전문’은 업무 범위를 찾는 검색어로 이해하고, 공인 전문자격처럼 표방하지 않습니다.",
      links: [
        { href: "/부산법인설립등기", label: "법인설립" },
        { href: "/부산임원변경등기", label: "임원변경" },
        { href: "/부산본점이전등기", label: "본점이전" },
        { href: "/부산유상증자등기", label: "유상증자" },
        { href: "/부산감자등기", label: "감자" },
      ],
    });
    extraSections.unshift({
      title: "공공기관 법인등기",
  ...
  if (page.slug === "부산등기법무사") {
    extraSections.unshift({
      title: "부산에서 등기전문 법무사를 찾을 때 업무부터 구분해야 하는 이유",
      body: "부동산·상속·법인·보상 등기는 등기부와 원인 서류가 다릅니다. ‘등기전문’ 검색어는 업무 범위를 찾는 표현으로 이해하고, 지금 바꾸려는 등기부부터 고른 뒤 해당 안내로 이어가면 됩니다. 공인 전문자격처럼 표방하지 않습니다.",
      links: [
  ...
  if (page.slug === "부산법무사비용" || page.slug === "부산법무사보수표") {
    extraSections.unshift({
      ...
      body: "법무사 수임료(보수)와 취득세·등록면허세·국민주택채권·인지·등기신청수수료는 성격이 다릅니다. 견적을 받을 때는 무엇이 포함되고 무엇이 별도인지 항목별로 확인하세요. 보정·출장·복대리가 있으면 추가될 수 있습니다. 확정되지 않은 금액은 사건별 확인이 필요하다고 표시합니다.",
      items: [
        "수임료·착수금·잔금(사건별 약정)",
        "등록면허세·취득세 등 세금",
        "국민주택채권·인지·증지",
      ...
      links: [
        ...
        { href: "/contact/inquiry?from=부산법무사비용&intent=비용 구성 확인", label: "준비서류와 비용 문의하기" },
      ],
    });
    extraSections.unshift({
      title: "전화 안내와 서류 확인 후 확정의 차이",
      body: "전화로는 대략적인 비용 구성 안내가 가능한 경우가 있습니다. 등기부·당사자·권리관계를 확인한 뒤에야 비용이 정해지는 경우도 많습니다. 지나치게 낮은 금액만으로 사무소를 고를 때는 포함 범위(말소·보정·출장·복대리)를 확인하세요.",
      items: [
        "대략 안내 가능: 보수와 공과금의 구성",
        "서류 확인 후: 확정 견적",
        "사건별 확인 필요: 고정 단가로 공시하지 않는 항목",
      ],
      links: [
...
export function buildPageDataFromBlogMeta(meta: ContentMeta): PageData {
  return createPageData({
    ...
    primaryKeywords: [...meta.tags, "다옴법무사사무소", meta.category],
...
export function buildPageDataFromCaseMeta(meta: ContentMeta): PageData {
  return createPageData({
    ...
    primaryKeywords: [...meta.tags, "다옴법무사사무소", "상담 사례"],
...
export function buildPageDataFromFaqMeta(meta: ContentMeta): PageData {
  ...
  return createPageData({
    ...
    primaryKeywords: [...meta.tags, "다옴법무사사무소", "FAQ"],
