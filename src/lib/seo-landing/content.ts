...
function uniqueAngleBlock(spec: SeoLandingSpec): string {
  ...
  if (spec.serviceId === "inheritance-renunciation") {
    const angles = [
      `${service}에서는 3개월 기한·가족관계·처분 이력을 먼저 맞춥니다.`,
      `${service} 접수 전에는 배우자·자녀 구성을 확인해 포기 효과를 가릅니다.`,
      `${service}는 가정법원 신고이며 부동산 등기 접수와 창구가 다릅니다.`,
      `${service} 비용은 법원 실비와 보수를 구분해 안내합니다.`,
      intent
        ? `${service} ${intent} 문의는 견적 숫자보다 기한·신청인 구성·추가 업무를 먼저 확정합니다.`
        : `${service} 일정은 서류 완비일과 가정법원 접수 가능일을 구분해 안내합니다.`,
...
function serviceFocusLines(serviceId?: string): string[] {
  const map: Record<string, string[]> = {
    ...
    "inheritance-renunciation": [
      ...
      "배우자·자녀 구성에 따라 포기 효과가 달라지므로 가족관계를 먼저 맞춥니다.",
...
function exclusiveClusterBody(spec: SeoLandingSpec): string {
  if (spec.serviceId === "inheritance-renunciation") {
    const clusters = [
      ...
      "배우자·자녀가 공동상속인인데 자녀만 전부 포기하면 배우자가 단독상속인이 되는 경우가 있습니다. 곧바로 부모·형제에게 넘어간다고 보지 않습니다.",
      "이미 예금·재산을 썼다면 단순승인 여부를 먼저 확인합니다. 처분 시기와 용도를 메모해 상담합니다.",
...
function buildSections(spec: SeoLandingSpec): PageSection[] {
  ...
  const costVariants =
    spec.serviceId === "inheritance-renunciation"
      ? [
          `${service} 비용은 가정법원 실비, 법무사 보수, 증명서·우편, 신청인 수, 미성년·해외·특별대리 여부에 따라 달라집니다. 부동산 취득세 견적 구조와는 다릅니다.`,
          `확정 금액은 상담 후 견적합니다. ${intent === "비용" || intent === "보수표" ? "수임과 실비를 구분해 안내합니다." : "비용만 궁금하면 상속포기 비용 안내를 먼저 보시면 됩니다."}`,
          `${region} ${service} 견적은 신청인 구성·기한 임박·추가 업무를 항목별로 나눕니다.`,
          `신청 가능성·3개월 기한·가족관계는 부산 상속포기 안내에서 먼저 확인하는 것이 좋습니다.`,
        ]
      : [
