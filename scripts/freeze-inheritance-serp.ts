/**
 * 상속 클러스터 베이스라인 동결.
 * 실행: npx --yes tsx scripts/freeze-inheritance-serp.ts
 *
...
const OUT = path.join(ROOT, "seo/inheritance-serp");
...
const INTENT_BY_PATH: Record<string, { primary: string; secondary: string }> = {
  ...
  "/부산법무사비용": { primary: "부산 법무사 비용", secondary: "cost" },
  "/상속포기비용": { primary: "상속포기 비용", secondary: "supporting" },
  "/부산가정법원상속포기": {
    primary: "부산가정법원 관할",
    secondary: "supporting-court",
  },
...
function main() {
  ...
  fs.writeFileSync(path.join(OUT, "before-routes.csv"), `${routeRows.join("\n")}\n`, "utf8");
  fs.writeFileSync(path.join(OUT, "before-meta.csv"), `${metaRows.join("\n")}\n`, "utf8");
  fs.writeFileSync(path.join(OUT, "before-links.csv"), `${linkRows.join("\n")}\n`, "utf8");
  fs.writeFileSync(path.join(OUT, "before-content.csv"), `${contentRows.join("\n")}\n`, "utf8");
  fs.writeFileSync(path.join(OUT, "after-protected.csv"), `${wideRows.join("\n")}\n`, "utf8");
  ...
  console.log(`Wrote ${pages.length} inheritance-related rows to seo/inheritance-serp/before-*.csv`);
