#!/usr/bin/env node
/**
 * TARGET SEO SURGERY — before/after static HTML snapshot + protected URL hash check.
 *
 * Usage:
 *   node scripts/target-seo-snapshot.mjs --phase=before
 *   node scripts/target-seo-snapshot.mjs --phase=after
 *   node scripts/target-seo-snapshot.mjs --compare
 */
...
const CACHE = path.join(ROOT, ".cache", "target-seo");
...
const TARGET_URLS = new Set([
  ...
  "/업무사례/울산상속등기법무사",
  "/부산보상등기",
  // intentional de-cannibalization (noindex + canonical → hubs)
  "/부산등기전문법무사",
  "/부산법인전문법무사",
]);
